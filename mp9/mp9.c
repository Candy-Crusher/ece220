...
//return: void
//side effects: vertex more than 100 cannot be included in the vertex_set
...
void find_nodes(locale_t *loc, vertex_set_t *vs, pyr_tree_t *p, int32_t nnum)
{
    ...
    if (nnum * 4 + 1 >= p->n_nodes)
    {
        ...
        if(in_range(loc, p->node[nnum].x, p->node[nnum].y_left)){
            ...
            //if not full, put the node into the vs set
...
//side_effect: when path longer than 501, fail
         ...
         path_t *path)
{
    ...
    int32_t id_copy[MAX_PATH_LENGTH];
    ...
    while(!(in_src(src,p_id))){
        ...
        id_copy[path->n_vertices++] = p_id;
        p_id = g -> vertex[p_id].pred;        
    }
    //add the start point in the src
    id_copy[path->n_vertices] = p_id;
    (path->n_vertices)++;

    //make the backward path forward
    for (int32_t i = 0; i < path->n_vertices; i++){       
        path->id[i] = id_copy[path->n_vertices -1 -i];
    }
    return 1;
}
