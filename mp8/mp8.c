...
#include "mp8.h"
/* introduction
	This file write three functions to complete flood fill operation
	An image is composed of an array of pixels. Each pixel has three 
	bytes of information in the RGB format. These bytes represent 
	the red, blue, and green levels of the pixel. The values for each 
	pixel range from 0 to 255. In a flood fill operation, the user 
	selects one pixel in the image along with an RGB color. In a painting
	analogy, the chosen color floods out in all directions from the 
	chosen pixel until it reaches a boundary. In the simplest case, such 
	a boundary might simply be a pixel of another color, such as white.
	One can implement flood fill recursively, although the number of stack 
	frames needed may be too large for typical image sizes, so you will 
	implement a version that limits the range of the flooding to about 35 
	pixels, thus limiting the total number of pixels to about 3,850.*/
...
basicFlood (int32_t width, int32_t height,
	    ...
	    uint8_t* outBlue)
{
	/*The wrapper should use outRed to record 
	  whether each pixel has been visited by the flood. 
	  Initially, no pixels have been visited
	  The first step is to fill the array with 0s*/
	memset(outRed, 0, width*height);

	//Then call basicRecurse (once)
	basicRecurse(width, height, inRed, inGreen, inBlue, startX, startY, outRed);

	//Finally, walk through the whole image: 
	//pixels marked in the marking array should be filled with the flood color. 
	//Pixels not marked in the marking array should be copied from the original image.
	for(int32_t y = 0; y < height; y++){
		for(int32_t x = 0; x < width; x++){
			if(outRed[x + width*y] != 0){
				outRed[x + width*y] = floodR;
				outGreen[x + width*y] = floodG;
				outBlue[x + width*y] = floodB;
			}else{
...
{
	uint32_t z = (r1 - r2)*(r1 - r2) + (g1 - g2)*(g1 - g2) + (b1 - b2)*(b1 - b2);
    if(z <= distSq){
		return 1;
	}else{
		return 0;
